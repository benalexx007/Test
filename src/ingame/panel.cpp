...
{
    // Preserve the panel's current position and size
    int px = getX();
    int py = getY();
    int pw = getWidth();
    int ph = getHeight();

    // If size is not set (first call from Start), use the provided parameters
    if (pw == 0 || ph == 0) {
        pw = winW;
        ph = winH;
    }

    // Recreate the panel at the previous position and reset its contents
    ...
    if (hasActive) {
        ...
        if (b) {
            ...
            b->setCallback([this]() {
                if (!userPtr) return;
                userPtr->logout();
                // After logout, immediately reset the UI to the registration/login view
                ...
                // Do not call onChangedCallback here so the panel remains open
            });
        }
    }
                         ...
                         bool isInGame, std::function<void()> onQuitGame, Game* gamePtr)
{
    ...
    // Button color and size configuration
    const SDL_Color btnCol = {0xf9, 0xf2, 0x6a, 0xFF};
    const int BtnW = 350;
    const int BtnH = 85;
    const int Padding = 16;
    ...
    // Base Y position below the SETTINGS title
    int baseY = titleLocalY + titleFontSize + 30;

    // ACCOUNT button: only shown when not in-game
    ...
    if (!isInGame) {
        ...
        if (accountBtn) {
            ...
            accountBtn->setCallback([this, user, winW, winH, onChanged]() {
                ...
                // Preserve current panel position
            });
        }
        yAfterAccount = baseY + BtnH + Padding;
    }

    // MUSIC button: located just below SETTINGS (or below ACCOUNT if present)
    ...
    // Next vertical position after MUSIC
    int currentY = yMusic + BtnH + Padding;

    if (isInGame) {
        // In-game: show RETURN on one line and QUIT on a separate line
        ...
        if (returnBtn) {
            ...
            returnBtn->setCallback([onChanged]() {
                // Close the panel and resume play
        ...
        Button* quitBtn = addButton(0, currentY, BtnW, BtnH, "QUIT",
            ...
            if (onQuitGame) {
                quitBtn->setCallback([onQuitGame]() {
                    std::cerr << "SettingsPanel: QUIT pressed (in-game)\n";
                    // Push a custom SDL user event so the Start screen can
                    // detect and reinitialize itself after the Game returns.
                    extern Uint32 gAppReturnToStartEvent; // declared in app_events.h
                    SDL_Event ev;
                    SDL_memset(&ev, 0, sizeof(ev));
                    ev.type = gAppReturnToStartEvent;
                    ...
                    // Also set a simple global flag so Start can detect
                    // the request even if the SDL event was lost during
                    // game cleanup.
                    extern bool gReturnToStartRequested;
                    gReturnToStartRequested = true;
                    if (onQuitGame) onQuitGame();
                });
            }
        }
    } else {
        // Outside the game: only a QUIT button under MUSIC to close the SETTINGS panel
