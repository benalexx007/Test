...
bool Stages::init(SDL_Renderer* rend, User* user_, int w, int h, std::function<void(char)> onSelectCb)
{
    ...
    // Set selected index based on the user's current stage
    if (user) {
        char stageChar = user->getStage();
        if (stageChar >= '1' && stageChar <= '3') {
            selected = static_cast<int>(stageChar - '1'); // '1' -> 0, '2' -> 1, '3' -> 2
        } else {
            selected = 0; // default to stage 1 if the stored stage is invalid
        }
    } else {
        selected = 0; // default to stage 1 when no user is present
    }
...
bool Stages::isIndexUnlocked(int idx) const
{
    ...
    // Stage 1 (idx 0) is always unlocked (even if not yet played)
    if (idx == 0) {
        return true;
    }
    
    // Stage 2 (idx 1) is unlocked if the user completed stage 1 (stage >= '2')
    // Stage 3 (idx 2) is unlocked if the user completed stage 2 (stage >= '3')
